...
- [x] 実Reinfolib APIキーでの疎通確認・実データ取込検証（完了：2026-09-22）：
  `reinfolib:check`（接続確認）、`app:update-muni-amounts`（実データ取込・40四半期分の
  バックフィルを含む、§9.6.2参照）、`app:poll-muni-amounts-update`（更新検知ポーリング、
  3.6秒で正常応答：次四半期2026-Q2は未公開のため待機判定）をいずれも実キーで実行し、
  正常動作を確認（詳細: §9.5・§9.6）
  - 補足: このテストで使用した実キーは2026-09-21のAIセッション内漏洩事故（§9.4参照）で
    露出したものであり、再発行依頼済みだが2026-09-22時点で `apps/api/.env` はまだ
    切り替わっておらず、引き続き旧キー（漏洩済み）のまま利用している。新キー確定後は
    速やかに `.env` と GitHub Secrets を更新すること（本番への反映は新キー確定後に
    行う、§9.7参照）

#### フェーズ3: 検証・最適化
- [x] 取込ジョブの所要時間実測（完了：2026-09-22。43.9秒/四半期。詳細: §9.2項目6・§9.5.1）
- [x] 直近四半期0件市区町村の確認（完了：2026-09-22。5市区町村を実在確認、うち檜原村は
  一時的0件と判明。詳細: §9.2項目7・§9.5.3）
- [x] エンドツーエンドテスト（EnterCheck → Map 画面遷移、2026-09-22）：依存を追加せず
  Node.js 標準テスト（`frontend/tests/enter-check-to-map.test.mjs`）で、EnterCheck のフォーム送信が
  `/search-result` に遷移する契約と地図画面の描画・アクセシブルな凡例を検証。
- [x] API レスポンスのパフォーマンス検証（完了：2026-09-22。33,446行の履歴を
  SQLiteインメモリDBに投入した `GET /api/muni/amounts` のウォームアップ後応答を
  1秒未満とする回帰テストを追加。239市区町村のレスポンス件数・形式も検証。
  地図描画の実ブラウザ計測は未実施）
- [x] アクセシビリティ監査（色覚多様性・キーボード操作、2026-09-22）：
  EnterCheck の不正な `body` 要素を `main` に修正し、「次へ」を submit、「キャンセル」を button
  としてキーボード操作可能にした。入力済み予算は disabled ではなく読み取り専用にし、Tab で内容を
  確認できる。地図画面では遷移直後に main へフォーカスを移し、凡例を色だけに依存しない文言と
  境界線で表現した。凡例は §7.3 の色覚多様性検証済みの青・アンバー・赤・2段階グレーを使用する。
...
### 9.7 未実施：本番環境への反映（2026-09-22時点）

§9.5・§9.6の検証・バックフィルは**すべてローカルSQLite環境**で実施したものであり、
本番（ConoHa WING・MySQL）には未反映。以下が残タスク：

- [ ] **新APIキーへの切替**：2026-09-21の漏洩事故（§9.4）を受けて再発行依頼済みだが、
  2026-09-22時点で `apps/api/.env`（ローカル）はまだ旧キー（漏洩済み）のまま。
  新キー確定後、ローカル`.env`とGitHub Secrets（`REINFOLIB_API_KEY`）の両方を更新すること
- [ ] **本番crontab切替**（§9.3参照、SSHアクセスが必要なため本エージェントでは実施不可）
- [ ] **本番DBへの10年分バックフィル**：新キー確定・crontab切替後、本番で
  `php artisan app:backfill-muni-amounts --periods=40` を実行し、ローカルと同様に
  10年分の履歴を投入する。**漏洩済みの旧キーで本番に反映することは避けること**
  （本番反映は必ず新キー確定後に行う）
