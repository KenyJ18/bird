...
- [ ] 実Reinfolib APIキーでの疎通確認：`app:update-muni-amounts` / `app:poll-muni-amounts-update`
  はいずれもローカルSQLite環境でフェイクAPIクライアントに差し替えた結合テストのみ実施済み。
  実キーでの実データ取得・集計値の妥当性は未確認（詳細:
  `docs/design/PHASE_2_ITEM8_REINFOLIB_CLIENT_FIX.md` §7.4）
